...
#define CCVM_OPCODES {"MOV", "PUSH", "POP", "SWAP", "ROTL", "ROTR", "DUP", "ALLOC", "REALLOC", "FREE", "SYSCALL", "ADD", "SUB", "MUL", "DIV", "BBA", "DEBUG", "EXIT"}
#define CCVM_REGISTERS { "a", "b", "c", "d" }

#define CCVM_INSTRUCTION_SET {\
    {"MOV", { { 0x00, { TokenType::REGISTER, TokenType::NUMBER } },\
              { 0x01, { TokenType::REGISTER, TokenType::REGISTER } },\
              { 0x02, { TokenType::REGISTER,  TokenType::ADDRESS } },\
              { 0x03, { TokenType::ADDRESS, TokenType::NUMBER } },\
              { 0x04, { TokenType::ADDRESS, TokenType::REGISTER } },\
              { 0x05, { TokenType::ADDRESS, TokenType::ADDRESS } } } },\
    {"PUSH", { { 0x06, { TokenType::NUMBER } },\
               { 0x07, { TokenType::REGISTER } },\
               { 0x08, { TokenType::ADDRESS } } } },\
    {"POP", { { 0x09, { } },\
              { 0x0A, { TokenType::REGISTER } },\
              { 0x0B, { TokenType::ADDRESS } } } },\
    {"SWAP", { { 0x0C, { } } } },\
    {"ROTL", { { 0x0D, { } } } },\
    {"ROTR", { { 0x0E, { } } } },\
    {"DUP", { { 0x0F, { } } } },\
    {"ALLOC", { { 0x10, { TokenType::REGISTER, TokenType::NUMBER } },\
                { 0x11, { TokenType::REGISTER, TokenType::REGISTER } },\
                { 0x12, { TokenType::REGISTER } },\
                { 0x13, { TokenType::NUMBER } } } },\
    {"REALLOC", { { 0x14, { TokenType::REGISTER, TokenType::NUMBER } },\
                  { 0x15, { TokenType::REGISTER, TokenType::REGISTER } },\
                  { 0x16, { TokenType::REGISTER } },\
                  { 0x17, { TokenType::NUMBER } } } },\
    {"FREE", { { 0x18, { TokenType::REGISTER } },\
               { 0x19, { } } } },\
    {"ADD", { { 0x30, { } } } },\
    {"SUB", { { 0x31, { } } } },\
    {"MUL", { { 0x32, { } } } },\
...
namespace CCA {
    ...
    uint64_t parseNumber(std::string &code, unsigned int &readingIndex) {
        std::string result = "";
        ...
        if (base == 2) {
            return std::stoull(result, 0, 2);
        } else if (base == 8) {
            return std::stoull(result, 0, 8);
        } else if (base == 16) {
            return std::stoull(result, 0, 16);
        }
    ...
    std::vector<Token> lexer(std::string code) {
        ...
        for (unsigned int readingIndex = 0; readingIndex < code.size(); readingIndex++) {
            ...
            if (isIgnorable(currentCharacter)) {
                ...
                uint64_t value = parseNumber(code, readingIndex);
    ...
    void pushNumeric(std::vector<unsigned char> &bytecode, const Token &t) {
        for (int i = 0; i < 8; i++) {
            unsigned char byte = (t.valNumeric >> (56 - 8 * i)) & 0xFF;
    ...
    void pushLabel(std::vector<unsigned char> &bytecode, const Token &t) {
        for (int i = 0; i < 8; i++) {
            unsigned char byte = (t.byteIndex >> (56 - 8 * i)) & 0xFF;
            bytecode.push_back(byte);
        }
    }
    ...
    void generateBytecode(std::vector<Definition> definitions, std::vector<Token> tokens, std::string fileName) {
        ...
        for (unsigned int i = 0; i < tokens.size(); i++) {
            ...
            if (opcode.valString == "POP")
                std::cout << " === Searching for instruction " << opcode.valString << " with " << arguments.size() << " arguments, " << possibleInstructions.size() << " possible instructions ===\n";
            for (int j = 0; j < possibleInstructions.size(); j++) {
                // check every instruction
                Instruction instr = possibleInstructions[j];
                // {"POP", { { 0x09, { } }, { 0x0A, { TokenType::REGISTER } }, { 0x0B, { TokenType::ADDRESS } } } },\

                if (instr.opcode == 0x09 || instr.opcode == 0x0A || instr.opcode == 0x0B) {
                    std::cout << " - Comparing with instruction " << (int) instr.opcode << " with " << instr.args.size() << " arguments\n";
                }

                // print what we are currently comparing
                // std::cout << " - Comparing with instruction " << (int) instr.opcode << " with " << instr.args.size() << " arguments\n";
                ...
                if (matching) {
                    ...
                    // print instr.opcode as hex value
                    std::cout << "" << std::hex << (int) instr.opcode << std::dec << "\n";
        ...
        for (int byteIndexCounter = 0; byteIndexCounter < bytecode.size(); byteIndexCounter++) {
            unsigned char byte = bytecode[byteIndexCounter];
            file.write((char *) &byte, 1);
            
            for (int i = 0; i < definitions.size(); i++) {
                if (definitions[i].found_at_index == byteIndexCounter + 1) {
